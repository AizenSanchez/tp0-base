...
class ProtocolBody:
    ...
    def ProtocolBodyFromBytes(bytes):
        offset = 0
        client_bet_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
        client_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
        client_bytes = bytes[offset:offset+client_size]
        name, lastname, dni, birthdate = ProtocolBody.deserialize_client(client_bytes)
        offset += client_size
        number_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
    ...
    def deserialize_client(bytes):
        offset =0
        name_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
        name = bytes[offset:offset+name_size].decode('utf-8')
        offset += name_size
        lastname_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
        lastname = bytes[offset:offset+lastname_size].decode('utf-8')
        offset += lastname_size
        dni_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
        dni = bytes[offset:offset+dni_size].decode('utf-8')
        offset += dni_size
        birthdate_size = int.from_bytes(bytes[offset:offset+4], byteorder='big')
        offset += 4
